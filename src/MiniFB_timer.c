...
mfb_timer_compensated_reset(struct mfb_timer *tmr) {
    ...
    if (actual_frame_ticks > max_frame_ticks) {
        tmr->accumulated_error_ticks = 0;  // Clear accumulated error
    ...
    // Accumulate error
    tmr->accumulated_error_ticks += error_ticks;

    // Apply correction if error exceeds threshold
    int64_t correction_ticks = 0;
    if (llabs(tmr->accumulated_error_ticks) > threshold_ticks) {
        correction_ticks = (int64_t) (tmr->accumulated_error_ticks * MAX_CORRECTION);
        tmr->accumulated_error_ticks -= correction_ticks;
    }
